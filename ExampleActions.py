...
class ExampleActions(UserActionsBase):
    ...
    def create_actions(self):
        self.add_track_action('recplay', self.sa_rec)
    ...
    def sa_rec(self, action_def, args):
        ...
        for ttrack in self.song().tracks:
            self.canonical_parent.log_message('looping through tracks, %s' % ttrack)
            if ttrack == track:
                self.log('playing_status: %s' % ttrack.clip_slots[clip].playing_status)
                self.log('is_recording: %s' % ttrack.clip_slots[clip].is_recording)
                if ttrack.clip_slots[clip].is_recording:
                    ...
                    clip += 1
                    break
        action = '%s/play %s' % (track_index, clip + 1)
        self.canonical_parent.clyphx_pro_component.trigger_action_list(action)
